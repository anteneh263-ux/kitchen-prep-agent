...
- **Component prep grouped by station** — one job per ingredient, aggregated
  ...
  one chopping board. Each job carries both the quantity to produce and the
  quantity to draw from stock.
...
- **8 component prep jobs across three stations** — butchery carries the heaviest
  load, then the cold station, then the hot station — alongside the 6 dish
  assembly tasks.
...
│   │   ├── prep.py                  # Today: consumption, shortfalls, dish + station tasks
